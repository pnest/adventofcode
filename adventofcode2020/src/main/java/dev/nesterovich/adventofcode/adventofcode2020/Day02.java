package dev.nesterovich.adventofcode.adventofcode2020;
...
public class Day02 {
    ...
    public static void main(String[] args) {
        new NumberOfOccurrences().printValid(PASSWORD_POLICY);
        new PositionsOfLetters().printValid(PASSWORD_POLICY);
    }

    static class NumberOfOccurrences {
        public void printValid(String[] rows) {
            int validRows = 0;
            for (String row : rows) {
                int prev = 0;
                int curr = 0;
                for (; curr < row.length() && Character.isDigit(row.codePointAt(curr)); curr++) ;
                int minAmount = Integer.valueOf(row.substring(prev, curr), 10);

                curr++;
                prev = curr;

                for (; curr < row.length() && Character.isDigit(row.codePointAt(curr)); curr++) ;
                int maxAmount = Integer.valueOf(row.substring(prev, curr), 10);

                curr++;
                prev = curr;

                int codePointExpected = row.codePointAt(prev);

                prev += ": ".length();

                int found = 0;
                for (; prev < row.length(); prev++) {
                    if (row.codePointAt(prev) == codePointExpected) {
                        found++;
                    }
                }

                if (found >= minAmount && found <= maxAmount) {
                    validRows++;
    ...
    static class PositionsOfLetters {
        public void printValid(String[] rows) {
            int validRows = 0;
            for (String row : rows) {
                int prev = 0;
                int curr = 0;
                for (; curr < row.length() && Character.isDigit(row.codePointAt(curr)); curr++) ;
                int firstPosition = Integer.valueOf(row.substring(prev, curr), 10);

                curr++;
                prev = curr;

                for (; curr < row.length() && Character.isDigit(row.codePointAt(curr)); curr++) ;
                int secondPosition = Integer.valueOf(row.substring(prev, curr), 10);

                curr++;

                int codePointExpected = row.codePointAt(curr);

                curr += ": ".length();

                if (
                        (row.codePointAt(curr + firstPosition) == codePointExpected && row.codePointAt(curr + secondPosition) != codePointExpected)
                                || (row.codePointAt(curr + firstPosition) != codePointExpected && row.codePointAt(curr + secondPosition) == codePointExpected)
                ) {
                    System.out.println("Row [" + row + "] is valid");
                    validRows++;
